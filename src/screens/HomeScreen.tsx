...
const HomeScreen = () => {
  ...
  const fetchWeeklyData = async () => {
    ...
    // Step 2: course info (start + duration)
    ...
    if (courseError || !courseData?.start_date) {
      console.error('Error fetching course info:', courseError?.message);
      return;
    }

    const startDateStr = courseData.start_date;

    // Step 3: weekly task counts (returns duration_weeks rows after your SQL change)
    ...
    const rows = (data ?? []) as WeeklyData[];
    setWeeklyData(rows);

    // Step 4: current week index (clamp to rows length)
    if (startDateStr && rows.length > 0) {
      const startDate = new Date(startDateStr);
      const today = new Date();
      const msPerWeek = 1000 * 60 * 60 * 24 * 7;

      const weeksSinceStart = Math.floor((today.getTime() - startDate.getTime()) / msPerWeek);
      const clampedWeekIndex = Math.min(Math.max(0, weeksSinceStart), rows.length - 1);
    ...
    useCallback(() => {
      ...
      const fetchPipelineSummary = async () => {
        ...
        if (!error && data) {
          ...
          const totalRevenue = data.reduce((sum: number, c: any) => sum + (c.pipeline_revenue || 0), 0);
