...
const TaskLogScreen = () => {
  ...
  const [courseStart, setCourseStart] = useState<Date | null>(null);
  const [courseEndExcl, setCourseEndExcl] = useState<Date | null>(null);
  ...
  useEffect(() => {
    const fetchTaskTypes = async () => {
      ...
      if (error) {
        ...
        setTaskTypes(data);
        if (data.length > 0) setTaskTypeId(data[0].task_type_id);
      }
    };
    ...
    const fetchActiveCourse = async () => {
      ...
      if (courseError || !course?.start_date) {
        console.error('Could not fetch course start_date', courseError?.message);
        return;
      }
      ...
      endExcl.setDate(endExcl.getDate() + weeks * 7);

      setCourseStart(start);
      setCourseEndExcl(endExcl);
    };
  ...
  const isWithinCourseRange = (d: Date) => {
    // If course dates aren't available, do not block logging.
    if (!courseStart || !courseEndExcl) return true;

    const day = new Date(d);
    day.setHours(0, 0, 0, 0);

    return day.getTime() >= courseStart.getTime() && day.getTime() < courseEndExcl.getTime();
  };
  ...
  const handleLogTask = async () => {
    ...
    if (!isWithinCourseRange(selectedDate)) {
      return showSnack('Selected date is outside your active course range');
    }
